...
This ISO is based on ArchLinux ISO modified to provide Installation Environment for cocoLinux

coco Archiso Sources:
https://github.com/C0C0B01/coco-live
...
Live environment will start now and let you install cocoLinux to disk.

Welcome to cocoLinux!
...
change_grub_version() {
    local _version="$1"
    sed -i "s/cocoLinux_VERSION=\".*\"/cocoLinux_VERSION=\"${_version}\"/" ${src_dir}/archiso/grub/grub.cfg
}
...
run_build() {
    ...
    mv "$outFolder/$_profile/cocoLinux-$(date --date="@${SOURCE_DATE_EPOCH:-$(date +%s)}" +%Y.%m.%d)-x86_64.iso" "$outFolder/$_profile/${iso_file}"
